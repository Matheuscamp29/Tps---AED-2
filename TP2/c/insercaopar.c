...
int comparePokemons(Pokemon *p1, Pokemon *p2) {
    ...
    int date1Valid = parseDate(p1->captureDate, &date1);
    int date2Valid = parseDate(p2->captureDate, &date2);
    int result;
...
void partialInsertionSort(Pokemon *array, int n, int k) {
    for (int i = 1; i < n; i++) {
        Pokemon key = array[i];
        int j = i - 1;
        comparisons++;
        while (j >= 0 && comparePokemons(&array[j], &key) > 0) {
            array[j + 1] = array[j];
            movements++;
            j--;
            if (j >= 0) {
...
int main() {
    ...
    clock_t start = clock();
    partialInsertionSort(selectedPokemons, selectedCount, K);
    clock_t end = clock();
    ...
    char logFileName[50];
    sprintf(logFileName, "%s_insercaoParcial.txt", matricula);
