// Quicksort by generation with tie-breaker by name
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
...
void quicksort(Pokemon *array, int low, int high) {
    if (low < high) {
        Pokemon pivot = array[high];
        movements++;
        int i = low - 1;
        for (int j = low; j <= high - 1; j++) {
            if (comparePokemons(&array[j], &pivot) <= 0) {
                i++;
...
int main() {
    ...
    quicksort(selectedPokemons, 0, selectedCount - 1);
