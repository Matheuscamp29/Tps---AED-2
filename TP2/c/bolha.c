...
void bubbleSort(Pokemon *array, int n) {
    int swapped;
    for (int i = 0; i < n - 1; i++) {
        swapped = 0;
        for (int j = 0; j < n - i - 1; j++) {
            if (comparePokemons(&array[j], &array[j + 1]) > 0) {
...
int main() {
    ...
    bubbleSort(selectedPokemons, selectedCount);    
    ...
    char logFileName[50];
    sprintf(logFileName, "%s_bolha.txt", matricula);
