...
//=============== Estrutura Fila com Alocação Flexível ================
...
// Função para criar uma nova fila circular com alocação flexível
...
void enqueue(FilaCircular* fila, Pokemon* p, int* contador) {
    if (fila->count == MAX_TAM_FILA) {
        ...
        // Não imprimir a remoção automática
        free(temp);
        fila->count--;
    }
    ...
    if (novo == NULL) {
        fprintf(stderr, "Erro ao alocar memória para um novo nó da fila.\n");
        exit(EXIT_FAILURE);
    }
    novo->p = p;
    novo->next = NULL;
    ...
    // Incrementar o contador e imprimir a média
    (*contador)++;
    printf("Média: %d\n", calcularMedia(fila));
}
...
int main() {
    ...
    // Criação da fila com alocação flexível
