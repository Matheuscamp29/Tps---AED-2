...
void inserirInicioLista(Lista* lista, Pokemon* p) {
    if (lista->n >= MAX_TAM_LISTA) {
        fprintf(stderr, "Erro ao inserir no início: lista cheia\n");
        exit(EXIT_FAILURE);
    }
...
void inserirFimLista(Lista* lista, Pokemon* p) {
    if (lista->n >= MAX_TAM_LISTA) {
        fprintf(stderr, "Erro ao inserir no fim: lista cheia\n");
        exit(EXIT_FAILURE);
    }
...
void inserirPosicaoLista(Lista* lista, Pokemon* p, int pos) {
    if (lista->n >= MAX_TAM_LISTA || pos < 0 || pos > lista->n) {
        fprintf(stderr, "Erro ao inserir na posição %d: posição inválida ou lista cheia\n", pos);
        exit(EXIT_FAILURE);
    }
